...
import { HomeIcon, DocumentPlusIcon, DocumentTextIcon, UserGroupIcon, Cog6ToothIcon, PresentationChartBarIcon, AcademicCapIcon, AdjustmentsHorizontalIcon, UserCircleIcon, ShieldCheckIcon, DocumentMagnifyingGlassIcon, ArchiveBoxIcon, BookOpenIcon, QueueListIcon, SparklesIcon as AiUtilsIcon, DocumentCheckIcon as PlagiarismIcon } from '@heroicons/react/24/outline'; // Removed CalendarDaysIcon
...
const Sidebar: React.FC = () => {
  ...
  const getIcon = (labelKey: string, isMobile: boolean = false): React.ReactNode => {
    ...
    switch (labelKey) {
      ...
      case LocalizationKeys.NAV_PLAGIARISM_CHECKER: return <PlagiarismIcon className={iconSizeClass} />; // New Icon for Plagiarism Checker
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 h-[70px] bg-primary-dark text-light-text flex items-center justify-around shadow-2xl z-50 border-t border-slate-700">
        {roleNavLinks.slice(0, 5).map((link: NavItem) => ( // Show max 5 items on mobile
          ...
          >
            ...
            <span className={`mt-1 truncate text-center ${ (link.labelKey === 'maqola_yuborish' || link.labelKey === 'maqolalarni_boshqarish_admin' || link.labelKey === 'foydalanuvchilarni_boshqarish' || link.labelKey === 'jurnallarni_boshqarish_admin' || link.labelKey === 'mening_jurnalim' || link.labelKey === LocalizationKeys.NAV_AI_DOCUMENT_UTILITIES || link.labelKey === LocalizationKeys.NAV_PLAGIARISM_CHECKER) ? 'text-[9px] leading-tight' : 'text-xs'}`}> {/* Adjust text size for longer labels */}
                {translate(link.labelKey, link.labelKey.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()))}
            </span>
          </NavLink>
        ))}
