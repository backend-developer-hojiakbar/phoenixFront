...
import { Language, UserRole, NotificationItem } from '../types';
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center space-x-2 sm:space-x-4">
            <div className="relative">
              <GlobeAltIcon className="h-5 w-5 text-medium-text absolute left-2 top-1/2 -translate-y-1/2 pointer-events-none sm:inline-block"/>
              <select
                value={language}
                onChange={(e) => setLanguage(e.target.value as Language)}
                className="bg-secondary-dark text-light-text border border-slate-700 rounded-md py-1.5 pl-8 pr-2 text-xs sm:text-sm focus:ring-accent-sky focus:border-accent-sky appearance-none"
                aria-label={translate('select_language', 'Select Language')}
              >
