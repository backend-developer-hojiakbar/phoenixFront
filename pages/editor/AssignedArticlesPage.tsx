...
import { Article, ArticleStatus, Journal, SelectOption } from '../../types';
...
const AssignedArticlesPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    api.get('/articles/')
        ...
        .catch(err => console.error("Failed to load assigned articles:", err))
        .finally(() => setIsLoading(false));
  }, []);

  const statusOptions: SelectOption[] = [
    { value: 'ALL', label: translate('all_statuses') },
    ...Object.values(ArticleStatus).filter(s => s !== ArticleStatus.PUBLISHED).map(s => ({ value: s, label: translate('status_' + s.toLowerCase() as any, s) }))
  ];

  const filteredArticles = useMemo(() => {
    return articles.filter(article => {
        const term = searchTerm.toLowerCase();
        const matchesSearch = article.title.toLowerCase().includes(term) || article.keywords.some(kw => kw.toLowerCase().includes(term));
            ...
            <Card key={article.id} className="hover:shadow-accent-purple/40">
              <div className="flex flex-col md:flex-row justify-between md:items-start mb-3">
                <div>
                    ...
                    <p className="text-xs text-medium-text">{translate(LocalizationKeys.AUTHOR_NAME_LABEL)}: {article.authorName} {article.authorSurname}</p>
                </div>
