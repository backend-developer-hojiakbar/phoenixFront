...
const AdminPrintedPublicationsPage: React.FC = () => {
  ...
  const fetchWriters = async () => {
    try {
      const response = await apiService.get('/users/?role=writer');
  ...
  const handleAssignWriter = async (orderId: number) => {
    if (!assignedWriter) {
      setError("Iltimos, yozuvchini tanlang.");
      return;
    }
    ...
    try {
      const response = await apiService.post(`/printed-publications/${orderId}/assign-writer/`, {
        writer_id: assignedWriter
      });
      
      setSuccessMessage("Buyurtma yozuvchiga tayinlandi!");
      setAssignedWriter('');
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <h1 className="text-3xl font-bold text-accent-sky flex items-center">
          ...
          {translate('admin_printed_publications_title', 'Bosma Nashrlar (Admin)')}
        </h1>
        <Button 
          variant="secondary" 
        ...
        >
          {translate('back_to_admin', 'Admin panelga qaytish')}
        </Button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                {orders.map(order => (
                  <div 
                    ...
                    onClick={() => {
                      ...
                      setAssignedWriter(order.assigned_writer?.id.toString() || '');
                    }}
                  >
        ...
        <div className="space-y-6">
          {selectedOrder ? (
            <Card title={translate('order_management', 'Buyurtmani boshqarish')} icon={<BookOpenIcon className="h-6 w-6 text-accent-sky"/>}>
              <div className="space-y-6">
                <div>
                ...
                <div>
                  <label className="block text-sm font-medium text-light-text mb-2">
                    {translate('assign_writer', 'Yozuvchiga tayinlash')}
                  </label>
                  <div className="flex space-x-2">
                    <select
                      value={assignedWriter}
                      onChange={(e) => setAssignedWriter(e.target.value)}
                      className="flex-1 bg-slate-800 border border-slate-700 rounded-md py-2 px-3 text-light-text focus:outline-none focus:ring-2 focus:ring-accent-sky"
                    >
                      <option value="">{translate('select_writer', 'Yozuvchini tanlang')}</option>
                      {writers.map(writer => (
                    ...
                    <Button
                      onClick={() => handleAssignWriter(selectedOrder.id)}
                      disabled={isSubmitting || !assignedWriter}
                      leftIcon={<UserGroupIcon className="h-4 w-4" />}
                    >
                      {translate('assign', 'Tayinlash')}
                    </Button>
                  </div>
                </div>
            ...
            <Card title={translate('order_management', 'Buyurtmani boshqarish')} icon={<DocumentTextIcon className="h-6 w-6 text-accent-sky"/>}>
...
export default AdminPrintedPublicationsPage;